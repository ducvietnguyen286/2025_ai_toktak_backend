...
def __config_logging(app):
    return setup_consumer_logging(app, "YOUTUBE")
...
def action_send_post_to_link(message):
    try:
        ...
        log_youtube_message(f"Send post {post_id} to link {link_id}")
        log_youtube_message(f"Send post to {link.type} of {link.social_type}")

        if not link or not post:
            log_youtube_message("ERROR: Link or post not found")
            return False
...
def process_message_sync(body, app):
    ...
    try:
        ...
        if action == "SEND_POST_TO_LINK":
            log_youtube_message(f"Processing SEND_POST_TO_LINK action {decoded_body}")
...
async def process_message_async(body, app):
    ...
    if not result:
        log_youtube_message("ERROR: Message processing failed")
    return result
...
async def main():
    ...
    log_youtube_message("Đang chờ message. Nhấn CTRL+C để dừng.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        log_youtube_message("ERROR: Consumer stopped by user")
    finally:
