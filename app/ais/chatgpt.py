...
- 각 문장은 띄어쓰기 포함 **글자 수 100자 이상 120자 이하**, **기본적으로 2줄 이내**로 구성합니다.  
  - 단, **예외적인 경우 최대 3줄까지 허용**할 수 있으나, 해당 경우는 매우 제한적으로 사용해야 하며 이미지 가독성을 해치지 않아야 합니다.  
- 각 문장은 실제 사용자의 감정이 담긴 자연스러운 SNS 말투로 작성해야 하며, 딱딱하거나 AI스러운 표현은 피해주세요.  
- 이모티콘은 사용하지 않습니다.  

- 아래 흐름을 반드시 따르세요:  
  1. [1번 이미지]  
    - 제품명 또는 브랜드명을 절대 언급하지 마세요.  
    - 스크롤을 멈추게 하는 문장으로 작성하고,  
    - **글자 수 32자 이하 + 4줄 이하 조건을 모두 충족**해야 하며, 둘 중 하나라도 초과하면 출력 실패로 간주합니다.  
    - 강한 **궁금증 유발 + 도파민 자극 티저형 문구**로 작성하세요.  
  2. [2번 이미지] 공감되는 일상, 감정, 문제 상황 표현  
  3. [3번 이미지] 변화의 시작, 제품이 작용하는 느낌  
  4. [4번 이미지] 확신을 주는 변화, 감정적/시각적 임팩트  
  5. [마지막 이미지] 감성적인 마무리 또는 제품 요약 (이때만 제품명 언급 가능)

[캡션 연결 흐름 구조 – 감정선 기반 작성 가이드]

- caption 배열은 5개의 독립 문장이 아닌, 사용자 경험을 축약한 **감정 흐름 기반의 짧은 스토리**처럼 작성해야 합니다.  
- 감정 전개는 아래의 흐름을 따르세요:  
  1 → 궁금증 유발  
  2 → 일상적 고민 또는 공감 포인트  
  3 → 새로운 변화의 시작  
  4 → 변화에 대한 확신, 감정적 피드백  
  5 → 감성 마무리 또는 요약, 제품 언급 가능  

- 문장 하나하나가 감정 흐름을 담되, 각 문장은 독립적으로도 의미가 통하도록 구성해야 합니다.  
