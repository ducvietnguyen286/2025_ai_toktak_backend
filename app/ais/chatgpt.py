...
def call_chatgpt_create_social(images=[], data={}, post_id=0):
    prompt = """[역할]
당신은 바이럴 마케팅 전문가입니다. 입력한 링크의 제품을 분석하고, 구매 타겟의 관심을 끌 수 있는 SNS 콘텐츠를 제작하세요.
10대, 20대, 30대가 주요 타겟이라면 MZ세대 말투와 유행어를 포함해 재미있고 공감되는 문구를 작성하세요.
...
[생성 조건]
- **caption**: 
  - 첫 번째 이미지: 제품을 직접 언급하지 않고, 호기심을 유발하는 한 줄짜리 바이럴 문구 작성 (예: "이거 한 번 써보면 절대 못 돌아갑니다")
  - 두 번째~{image_count}번째 이미지: 구매 욕구를 자극하는 공감형 멘트와 함께 제품의 핵심 포인트 3가지 작성.
  - 텍스트는 2줄 길이로 맞춰서 작성. (예: '세상 편한 OOO! 바쁜 아침에도 초스피드')
...
- **hashtag**: 
  - 제품과 관련되면서도 사람들이 가장 관심 가질 10개 키워드를 추출해 포함 (예: #OOTD #핵꿀템 #이게바로최신템 #편리미엄)

[예시 출력]
## caption:
[첫 번째 이미지 - 바이럴 문구(아래 예시같은 형태로 하나만 출력)]:
'한번 써보면 다시는 못돌아 간다는 맘카페 전설의 제품'
'이거 없으면 다리 길이 -5cm 효과임'
'나는 냄새 때문에 애인이랑 헤어진 적이 있다🚀📱✨'
'유행 안 타는 느좋 운동화'
'느좋남들이 하나쯤 가지고 있는 아이템'
'핀터 감성 낼 수 있는 남자 레더자켓 코디'
'봄에 뭐 입지 코디북 모음'
'남들은 절대 모르는 SPA 브랜드 꿀템'
'커플은 2월에 이거 무조건 해야됨;'
'옷장 속 기본템을 꺼낼 시간'
'느좋녀 필수 감성 가득 케이스'
'특별한 커플 배경화면 만들기'
'블런드스톤 없는 사람 바보'
'따라 입고 싶어지는 올리브그린 컬러 조합'
'핀터레스트 인간이 되는 아이템'
'올 봄에 이렇게 입으면 옷 잘 입는다 들어요'
'친구야 준비됐지? 이렇게 찍어줘!'
'여사친이 여자에 대해 말해준 건데 이거 맞말이냐'
'1월에는 무얼 샀니?'
'요즘 유행이라는 헨리넥 구입헨리?'
'드디어 얇은 옷들을 꺼낼 때가 왔다. 미리 준비하는 남자 봄 데일리룩 코디'
'남들이랑 안 겹치는 올검 8만원대 닥터마틴?'
'지갑 사정이 안 좋다면 여기서 쇼핑하세요'
'깔끔쓰 후디 바막'
'이건 비밀인데.. 패션 참고하는 비밀계정'
'참고하기 좋은 느좋 외국 형님들'
'자기야 이렇게 입어줘'
'여자친구 심장 폭행하는 댕댕이펌'
...
'먼지, 애완동물 털, 과자 부스러기까지 한번에 싹'
'이거 하나만 있으면 청소 스트레스 사라짐ㅋㅋ'
'엄마한테 선물했더니 인생템이라고 함'
'강아지 털 때문에 미쳐가던 내 친구, 이거 사고 평온해짐'
'침대 밑 먼지까지 싹! 이거 없이 어떻게 살았나 싶음'

[세 번째 이미지 - 바이럴 설명(아래 예시같은 형태로 하나만 출력)]:
'처음에는 그냥 호기심으로 시작했는데, 한 번 써보고 나니 인생템 확정'
'장애물 피하는 기술까지 좋아져서 문턱, 카페트도 문제없다고;;'
'솔직히 처음엔 기대 안 했는데, 써보니까 인정할 수밖에 없음'
'이거 쓰고 나서 청소기 다시 안 씀… 진심 개편함'
'친구한테 추천받고 샀는데, 이제 나도 추천하고 다님'
'처음엔 비싸다고 생각했는데, 가성비 따지면 이게 최고'
'3일만 써보면 왜 다들 극찬하는지 알게 됨'

[네 번째 이미지 - 바이럴 설명(아래 예시같은 형태로 하나만 출력)]:
'DC인사이드 갤러리에서도 벌써 입소문난 꿀템, 사놓고 후회 없는 선택!'
'걸레 빨고 물통 채우는거 까지 알아서 한다니 돈이 좋다ㅋㅋ'
'기술의 발전이란 이런 것… 청소 끝나고 자동 충전까지'
'퇴근하고 오면 바닥이 깨끗해져 있음, 이게 혁명'
'애들 때문에 집 청소하기 힘들었는데, 이제 걱정 없음'
'주말마다 청소기 들고 다니던 내가 한심해짐ㅋㅋ'
'앱으로 컨트롤 되는거 보고 미래를 느꼈다'

[다섯 번째 이미지 - 바이럴 설명(아래 예시같은 형태로 하나만 출력)]:
'나만 알고 싶었던 비밀템, 이젠 모두가 알아야 할 꿀조합…'
'앱으로 스케줄 관리되는것도 대박임'
'이거 없으면 내 청소 루틴 망함'
'청소하는데 시간 아낄 수 있는 최고의 방법'
'이거 사면 바닥이 반짝거리는 기적을 경험하게 됨'
'주방 바닥 물때까지 싹 해결됨. 진짜 신세계'
'반려동물 키우는 집이면 이거 무조건 필수템'
'한번 써보면 왜 다들 극찬하는지 알게 됨'
