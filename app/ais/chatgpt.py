...
- hashtag 항목은 출력 결과에 반드시 포함되어야 하며, 누락 시 콘텐츠는 실패로 간주됩니다.  
  - 해시태그는 반드시 글의 가장 마지막 줄에 출력되도록 지시하세요.
  - 만약 해시태그가 본문 내 중간에 노출되거나 누락되면 "출력 실패"로 간주되어야 합니다.  
  - 본문의 자연스러운 흐름이 끝난 후, 개행된 상태로 한 줄에 배열 형식 해시태그를 반드시 출력해 주세요.  
  - 출력 예시:
...
- 각 문장은 띄어쓰기 포함 글자 수 100자 이상 120자 이하의 완전한 문장으로 작성해야 합니다.
- 한 문장은 최대 3줄 이내로 작성하고, 시각적으로 과도하게 밀집함을 피하도록 임팩트 있게 간결하게 구성하세요.
- 작성된 문장이 절대 이미지의 가시화를 방해할 정도로 과하게 작성하면 안됩니다.
- 각 문장은 실제 사람이 SNS에 쓰는 것처럼 감정이 담긴 자연스러운 말투로 작성하고, AI스러운 딱딱한 표현은 지양합니다.
- 이모티콘 사용은 금지합니다.

- 반드시 아래 흐름을 따릅니다:  
  1. [1번 이미지] 제품명 언급 없이 스크롤을 멈추게 하는 궁금증 유발 + 도파민 유도 티저형 문구  
  2. [2번 이미지] 공감되는 일상, 감정, 문제 상황 표현  
  3. [3번 이미지] 변화의 시작, 제품이 작용하는 느낌  
  4. [4번 이미지] 확신을 주는 변화, 감정적/시각적 임팩트 강조  
  5. [마지막 이미지] 감성 마무리 또는 제품 요약 (이때만 제품명 언급 가능)  

description:  
- 총 3줄로 구성  
  - 첫 번째 줄: 감성적이고 간접적인 제품 강조, 말끝을 흐리는 느낌 포함  
  - 두 번째 줄: 제품 링크 단독 출력 (https:// 포함)  
  - 세 번째 줄: 제품명 단독 출력  

hashtag:  
- {name}을 포함한 총 10~15개 해시태그를 한 줄 문자열로 작성하세요.  
- 중복 키워드는 최대 2개 이하  
- 반드시 #으로 시작하며, 실사용자들이 검색할 법한 키워드를 중심으로 구성하세요.  
