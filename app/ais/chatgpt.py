...
- 각 항목은 **텍스트** 또는 `"IMAGE_URL_0"` 형식의 이미지 인덱스입니다.
    ...
    9. **해시태그** – 한 줄로 이어진 10~15개의 해시태그 (예: `#뷰티템 #립밤추천 #촉촉립밤 ...`)
...
- 각 문장은 띄어쓰기 포함 글자 수 100자 이상 120자 이하, 기본적으로 2줄 이내로 구성합니다.  
  - 단, 예외적인 경우 최대 3줄까지 허용할 수 있으나, 해당 경우는 매우 제한적으로 사용해야 하며 이미지 가독성을 해치지 않아야 합니다.  
...
- 아래 흐름을 반드시 따르세요:  
  1. [1번 이미지]  
    ...
    - 글자 수 32자 이하 + 줄 수 4줄 이하 조건을 반드시 만족해야 하며, 하나라도 초과되면 출력 실패로 간주합니다.   
...
- caption 배열은 5개의 독립 문장이 아닌, 사용자 경험을 축약한 감정 흐름 기반의 짧은 스토리처럼 작성해야 합니다.  
- 감정 전개는 아래의 흐름을 따르세요:  
  1 → 궁금증 유발  
  2 → 일상적 고민 또는 공감 포인트  
  3 → 새로운 변화의 시작  
...
```json
{
  "response": {
    ...
    "description": "향은 결국 나를 설명하는 방식이더라고…\\n[지금 클릭하면 숨겨진 할인 공개] https://example.com\\n시그니처 퍼퓸 오 드 뚜왈렛",
