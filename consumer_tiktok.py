...
async def process_message_with_retry(message: IncomingMessage, app, semaphore):
    ...
    async with message.process():  # Đảm bảo ACK message khi xử lý xong
        ...
        try:
            ...
            log_tiktok_message(f"Message processing failed after retries: {e}")
            return False
...
async def connect_rabbitmq_with_retry(rabbitmq_url, max_attempts=5):
    ...
    while attempt < max_attempts:
        try:
            ...
            log_tiktok_message("Connected to RabbitMQ successfully.")
            return connection
        except Exception as e:
            attempt += 1
            log_tiktok_message(f"Connection attempt {attempt} failed: {e}")
