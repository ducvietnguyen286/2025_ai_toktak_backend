...
RABBITMQ_QUEUE_TWITTER = os.environ.get("RABBITMQ_QUEUE_TWITTER") or "hello"
...
def action_send_post_to_link(message):
    try:
        ...
        log_twitter_message(f"Received message: {message}")

        if not link or not post:
            log_twitter_message("ERROR: Link or post not found")
            return False
...
def process_message_sync(body, app):
    ...
    try:
        ...
        if action == "SEND_POST_TO_LINK":
            log_twitter_message(f"Processing SEND_POST_TO_LINK action {decoded_body}")
        ...
        log_twitter_message(f"ERROR: Error processing message: {str(e)}")
        return False
...
async def process_message_async(body, app):
    ...
    if not result:
        log_twitter_message("ERROR: Message processing failed")
    return result
...
async def process_message_with_retry(message: IncomingMessage, app, semaphore):
    ...
    async with message.process():  # Đảm bảo ACK message khi xử lý xong
        ...
        log_twitter_message(f"Received message: {body}")
        try:
            async with semaphore:
                result = await process_message_async(body, app)
            return result
        except Exception as e:
            log_twitter_message(f"Message processing failed after retries: {e}")
            return False
...
async def connect_rabbitmq_with_retry(rabbitmq_url, max_attempts=5):
    ...
    while attempt < max_attempts:
        try:
            ...
            log_twitter_message("Connected to RabbitMQ successfully.")
            return connection
        except Exception as e:
            attempt += 1
            log_twitter_message(f"Connection attempt {attempt} failed: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        log_twitter_message("ERROR: Consumer stopped by user")
    finally:
